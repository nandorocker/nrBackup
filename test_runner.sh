...
check_bats() {
    if [[ ! -f "$BATS_BIN" ]]; then
        ...
        echo -e "${YELLOW}Please ensure Bats is installed in test/bats/${NC}"
        echo ""
        echo "To install Bats as a git submodule:"
        echo "  git submodule add https://github.com/bats-core/bats-core.git test/bats"
        echo "  git submodule add https://github.com/bats-core/bats-support.git test/test_helper/bats-support"
        echo "  git submodule add https://github.com/bats-core/bats-assert.git test/test_helper/bats-assert"
        exit 1
    fi
}
